...
def download(url):
    ...
    file_name = '../extensions/crx_files/' + chrome_app_id
...
if __name__ == "__main__":
    ...
    urls = set([r['url'] for r in TinyDB('./db.json').table('newURLs').all()][:10])
    urls |= set([each[:each.index('?')] for each in open('category_addons.txt', 'r').readlines()])
    urls |= set([each.strip() for each in open('collectURL.txt', 'r').readlines()])
    print("Total URLs ", len(urls))
    
    toContinue = input("Enter 'y' to contine or anything else to exit")
    if toContinue.lower()[0] == 'y':
        results = pool.map(download, list(urls))
        pool.close()
        pool.join()
        print('Done')
    else:
